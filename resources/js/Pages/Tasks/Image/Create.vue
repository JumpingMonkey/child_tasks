<template>
    ...
    <Box v-if="task.images.length" class="mt-4">
        ...
        <section class="mt-4 grid md:grid-cols-2 gap-4">
            <div v-for="image in task.images" :key="image.id"
                ...
                <Link :href="route('child.task.image.destroy', {task: prop.task.id, image: image})"
...
const upload = () => form.post(route('child.task.image.store', {task: prop.task}),
        {
